...
export default class SlotMachine extends EventEmitter {
    ...
    public multiWinPattern(spinResult: SpinResult): number[][] {
        ...
        if (spinResult.win?.winningPattern) {
    ...
    public bigWinLevel(roundResult: RoundResult): number {
        const winMultiplierRatio: number = roundResult.spins[roundResult.spins.length - 1].currentTotalWinValue / roundResult.totalBet;
        let level: number = -1
