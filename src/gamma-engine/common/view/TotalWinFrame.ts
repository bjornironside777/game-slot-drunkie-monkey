...
export default class TotalWinFrame extends Container {
    ...
    constructor(le: LayoutElement) {
        ...
        this.setValue(0);
        autoscaleText(this.tfTotalWin, parseInt(this.tfTotalWin.style.fontSize.toString()),300,70)
    ...
    private customClassElementCreate(le: LayoutElement): unknown {
        ...
        switch (le.customClass) {
            case 'TotalWinGlow':
                instance = new Spine(AssetsManager.spine.get('total-win-glow'));
                break;
            case 'ValueText':
    ...
    public setValue(value: number, animated: boolean = false) {
        ...
        if(animated) {
            ...
            autoscaleText(this.totalWinValue.tfValue,30,300,70)
