...
export default class StatusComponent extends  Container{
    ...
    constructor(le, private maxFontSize: number = 20) {
        super();
    ...
    public set winValue(value:number){
        this.statusWin.text.tfValue.text = `${this.wallet.getCurrencyValue(value,true)}`
        autoscaleText(this.statusWin.text.tfValue,26,126,52)
    }
    private onWalletBalanceChanged(){
        ...
        autoscaleText(this.statusBalance.text.tfValue,26,140,52)
    ...
    private onBetValueChanged(){
        this.statusBet.text.tfValue.text = `${this.wallet.getCurrencyValue(this.slotMachine.totalBet* this.wallet.coinValue * Wallet.denomination,true)}`;
        autoscaleText(this.statusBet.text.tfValue,26,140,52)
